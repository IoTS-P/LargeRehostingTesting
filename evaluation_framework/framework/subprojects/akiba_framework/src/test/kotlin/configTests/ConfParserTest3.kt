...
object ConfParserTest3 {
    @Test
    fun testParse() {
        Path.of("/tmp/akiba_test_3.json").writeText(testConfig_3)
        check (Path.of("/tmp/akiba_test_3.json").exists()) {
            "File /tmp/akiba_test_3.json failed to create"
        }
        Path.of("/tmp/akiba_test_3a.json").writeText(moduleConfig)
        check (Path.of("/tmp/akiba_test_3a.json").exists()) {
            "File /tmp/akiba_test_3a.json failed to create"
        }

        Configurator.reconfigure(File("configs/log4j2.xml").toURI())

        Main.mainConfigPath = "/tmp/akiba_test_3.json${ConfigManager.KEY_SEPARATOR}/main"
